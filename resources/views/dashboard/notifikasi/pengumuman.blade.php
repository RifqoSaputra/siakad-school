                ...
                <div class="ann-filter" data-filter="target_role">
                    ...
                    <div class="ann-filter__menu">
                        @foreach (['all' => 'Semua', 'guru' => 'Guru', 'ortu' => 'Ortu'] as $val => $label)
                            <button class="ann-filter__option {{ $val === 'all' ? 'ann-filter__option--divider' : '' }}"
                                data-value="{{ $val }}">
                ...
                <div class="ann-filter" data-filter="status">
                    ...
                    <div class="ann-filter__menu">
                        @foreach (['any' => 'Semua', 'sent' => 'Dikirim', 'scheduled' => 'Dijadwalkan', 'draft' => 'Draft'] as $val => $label)
                            <button class="ann-filter__option {{ $val === 'any' ? 'ann-filter__option--divider' : '' }}"
                                data-value="{{ $val }}">
            ...
            <div class="ann-header__right">
                <div class="ann-pagination">
                    ...
                    <div class="ann-pagination__arrows">
                        <button class="ann-icon-btn" data-nav-url="{{ $pengumuman->previousPageUrl() }}"
                            @disabled(!$pengumuman->previousPageUrl())>
                            <span class="material-symbols-rounded">chevron_left</span>
                        </button>
                        <button class="ann-icon-btn" data-nav-url="{{ $pengumuman->nextPageUrl() }}"
                            @disabled(!$pengumuman->nextPageUrl())>
                ...
                @forelse ($pengumuman as $row)
                    @php
                        $attachments = $row->attachments->map(function ($att) {
                            ...
                            $label =
                                $att->file_name ??
                                ($att->nama_file ?? basename($att->file_path ?? ($att->path ?? 'Lampiran')));
                            $url =
                                $att->file_path ?? $att->path
                                    ? asset('storage/' . ($att->file_path ?? $att->path))
                                    : null;
                            ...
                            @if ($attachments->count())
                                <div class="ann-attachments">
                                    @foreach ($attachments as $chip)
                                        ...
                                        @endphp
                                        @if ($chip['url'])
                                            <a class="ann-chip {{ $isPdf ? 'ann-chip--pdf' : '' }}"
                                                href="{{ $chip['url'] }}" target="_blank" rel="noopener">
                                                <span
                                                    class="material-symbols-rounded {{ $iconClass }}">{{ $chip['icon'] }}</span>
    ...
    <div class="ann-modal" id="modal-detail">
        ...
        <div class="ann-modal__card">
            <div class="ann-modal__header">
                ...
                <button class="ann-icon-btn" data-close="detail"><span
    ...
    <div class="ann-modal" id="modal-form">
        ...
        <div class="ann-modal__card ann-modal__card--form">
            <div class="ann-modal__header">
                ...
                <button class="ann-icon-btn" data-close="form"><span
                        class="material-symbols-rounded">close</span></button>
            </div>
            ...
            <form id="announcement-form" method="POST" action="{{ route('admin.pengumuman.store') }}"
                ...
                <div class="ann-form">
                    ...
                    <div class="ann-sendmode">
                        <label class="ann-sendmode__option" for="send_mode_now">
                            ...
                            <div class="ann-sendmode__text">
                                <div class="ann-sendmode__title">Saat ini, <span
                                        class="ann-sendmode__date">{{ now()->locale('id')->translatedFormat('j M Y') }}</span>
                                </div>
                            </div>
                        </label>
                    ...
                    <div class="ann-field-inline" id="schedule_section" style="display:none;">
                        <div class="ann-field">
                            ...
                            <div class="ann-datetime">
                                <input type="hidden" class="ann-input ann-input--datetime" id="scheduled_input"
                                    name="scheduled_for">
                                <div class="ann-datetime__display" id="scheduled_display" tabindex="0">
                                    <input type="text" id="scheduled_display_text" name="scheduled_display_text"
                                        class="ann-datetime__input" placeholder="Pilih tanggal" autocomplete="off">
                                ...
                                <div class="ann-datetime__popover" id="scheduled_popover">
                                    <div class="ann-datetime__header">
                                        ...
                                        <div class="ann-datetime__nav">
                                            <button type="button" data-month-nav="prev" class="ann-icon-btn"><span
                                                    class="material-symbols-rounded">chevron_left</span></button>
                                            <button type="button" data-month-nav="next" class="ann-icon-btn"><span
                        ...
                        <div class="ann-field">
                            ...
                            <div class="ann-datetime__display ann-datetime__display--time">
                                <input type="text" id="scheduled_time_input" name="scheduled_time_input"
                                    class="ann-input ann-input--time" inputmode="numeric" placeholder="00:00"
                                    autocomplete="off">
                    ...
                    <div class="ann-field" id="field-title">
                        ...
                        <input type="text" name="judul" class="ann-input" id="title-input" required
                    ...
                    <div class="ann-field" id="field-body">
                        ...
                        <textarea name="isi_pengumuman" rows="6" class="ann-textarea" id="body-input" required
                    ...
                    <div class="ann-field">
                        ...
                        <div class="ann-attach">
                            ...
                            <div class="ann-attach__error" id="attachments-error" style="display:none">Ukuran lampiran
                                melebihi 10MB, silahkan unggah kembali.</div>
                            <input type="file" id="attachments_input" name="attachments[]" multiple
                                style="display:none" accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg,.gif">
                        </div>
                ...
                <div class="ann-modal__footer">
                    ...
                    <button type="button" class="ann-btn ann-btn--secondary" id="btn-save-draft" disabled>Simpan
                        Draf</button>
    ...
    <div class="ann-modal" id="modal-confirm-close">
        ...
        <div class="ann-modal__card ann-modal__card--form">
            <div class="ann-modal__header">
                ...
                <button class="ann-icon-btn" data-close="confirm-close"><span
                        class="material-symbols-rounded">close</span></button>
            </div>
    ...
    <div class="ann-modal" id="modal-delete">
        ...
        <div class="ann-modal__card ann-modal__card--form">
            <div class="ann-modal__header">
                ...
                <button class="ann-icon-btn" data-close="delete"><span
                        class="material-symbols-rounded">close</span></button>
            </div>
            <div class="ann-modal__body">
